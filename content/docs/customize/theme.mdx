---
title: React Theme - Saastack React UI
description: Learn how you can change the Tailwind CSS classes used by the components in Saastack React UI
---
...
Saastack React UI is built on top of Tailwind CSS and it uses Tailwind CSS classes to style the components. This means that you can customize the components by changing the Tailwind CSS classes used by the components.
...
You can create a custom theme and pass it to the `<SaastackReactUI>` component. The custom theme will be merged with the default theme, and the resulting theme will be used by the components.

```tsx
import type { CustomSaastackReactUITheme } from 'flowbite-react';
import { SaastackReactUI } from 'flowbite-react';

const customTheme: CustomSaastackReactUITheme = {
  button: {
    color: {
...
export default function MyPage() {
  return (
    <SaastackReactUI theme={{ theme: customTheme }}>
